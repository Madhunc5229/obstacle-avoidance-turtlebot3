 ...
 * @brief this file contains all the implementations needed for obstacle
 * avoidance
 * @version 0.1
 * @date 2022-12-05
 *
...
ObsAvoid::ObsAvoid(const std::string &node_name) : Node(node_name) {
  ...
  subscriber_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
      "scan", 1,
      std::bind(&ObsAvoid::bot_callback, this, std::placeholders::_1));
}
...
bool ObsAvoid::obs_detected(const sensor_msgs::msg::LaserScan::ConstPtr &msg) {
  ...
  for (auto i : degrees) {
    if (msg->ranges[i] < 0.4) {
      RCLCPP_WARN_STREAM(rclcpp::get_logger("rclcpp"), "Obstacle detected");
      return true;
    }
  }
  RCLCPP_WARN_STREAM(rclcpp::get_logger("rclcpp"), "No Obstacle");
  return false;
}
